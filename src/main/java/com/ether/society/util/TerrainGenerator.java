package com.ether.society.util;
...
public class TerrainGenerator {
    ...
    public void generate(World world) {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                double latitudeFactor = 1.0 - (double)Math.abs(y - height/2) / (height/2); // 1.0 at equator, 0.0 at poles
    ...
    private double generateNoise(int x, int y, double scale) {
        ...
        // Using Math.sin/cos for deterministic pseudo-random patterns
        return Math.sin(x * scale) * Math.cos(y * scale);
    }

    private Biome determineBiome(double elevation, double rainfall) {
        if (elevation < -0.2) return Biome.OCEAN;
        if (elevation < 0.0) return Biome.BEACH;
        
        if (elevation > 0.8) {
            if (rainfall > 0.5) return Biome.SNOW;
            return Biome.MOUNTAINS;
        }
        
        if (rainfall < 0.2) return Biome.DESERT;
        if (rainfall < 0.5) return Biome.PLAINS;
        if (rainfall < 0.8) return Biome.FOREST;
